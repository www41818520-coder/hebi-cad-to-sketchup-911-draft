...
module CodexSketchUpBridge
  ...
  HOST = '127.0.0.1'
  BASE_PORT = 17_860
  CONFIG_PATH = File.join(__dir__, 'codex_su_bridge_config.json')
  ...
  def registry_dir
    ...
    configured.empty? ? File.join(Dir.home, 'codex_su_bridge_registry') : configured
  end
...
unless file_loaded?(__FILE__)
  UI.menu('Plugins').add_submenu('Codex Bridge').add_item('Restart Bridge') do
    CodexSketchUpBridge.stop
    CodexSketchUpBridge.start
  end
  CodexSketchUpBridge.start
